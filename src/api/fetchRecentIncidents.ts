...
export const fetchRecentIncidents = async (): Promise<Incident[]> => {
    const fetchIncidents = async (sheetName: string): Promise<Incident[]> => {
      ...
      if (!response.ok) {
        console.error(`Failed to fetch incidents from ${sheetName}`, response.statusText);
        return [];
      }
    ...
    try {
      const suffolkIncidents = await fetchIncidents("Suffolk%20County%20Working%20Fire%20Total");
      const nassauIncidents = await fetchIncidents("Nassau%20County%20Working%20Fire%20Total");

...
const DepartmentLookup: React.FC = () => {
  ...
  const [departmentData, setDepartmentData] = useState<Record<string, any> | null>(null);
  const [incidents, setIncidents] = useState<Incident[]>([]);
  const [deptStories, setDeptStories] = useState<Story[]>([]);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);

  const apiKey = process.env.REACT_APP_API_KEY as string;
  const incidentsSpreadsheetId = "1m1yjSvmhY0HbAePiA8e4td95l9lqOgFoZmlIu8wBTqU";
  ...
  const handleSearch = async () => {
    ...
    const fetchDepartments = async (sheetName: string): Promise<Record<string, any>[]> => {
    ...
    const fetchIncidents = async (
      ...
      const incidents = rows
        .map((row) => {
          const incident = Object.fromEntries(headers.map((key, i) => [key, row[i]])) as unknown as Incident;
          return incident;
        })
        .filter(
          (incident) =>
            ...
            incident.Latitude &&
            incident.Longitude &&
            new Date(incident["Fire Date"]).getFullYear().toString() === year // Filter by year
        );
    ...
    const checkDepartment = async (
      ...
      const matchingDepartment = departments.find(
        (dept) =>
          dept.Department.toLowerCase().includes(department.toLowerCase()) &&
          dept.Year === year
      );
    ...
    let departmentDetails = await checkDepartment("Suffolk", "Suffolk Departments");
    if (!departmentDetails) {
      departmentDetails = await checkDepartment("Nassau", "Nassau Departments");
    }
    ...
    if (departmentDetails) {
      ...
      const fetchedIncidents = await fetchIncidents(incidentsSheet, departmentDetails.Department);
      ...
      const fetchStories = async (): Promise<Story[]> => {
        ...
        return rows.map((row) =>
          Object.fromEntries(headers.map((key, i) => [key, row[i]])) as unknown as Story
        );
      };
      ...
      const relatedStories = stories.filter((story) =>
        (story[departmentHeader as keyof Story] as string)?.toLowerCase().trim() ===
        ...
        <>
          ...
          {incidents.length > 0 && (
            <>
              ...
              <DepartmentMap incidents={incidents} stories={deptStories} />
            </>
          )}
        </>
      )}
